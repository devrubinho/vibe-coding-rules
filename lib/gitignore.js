...
const GITIGNORE_ENTRIES = ['.task-flow', 'graphify-out'];
...
function appendTaskFlowEntries(content) {
  const normalized = stripTaskFlowGitignoreLines(content);
  const suffix = normalized.length > 0 ? '\n' : '';
  return `${normalized}${suffix}${GITIGNORE_ENTRIES.join('\n')}\n`;
}
...
async function updateGitignore(targetPath) {
  ...
  if (!fs.existsSync(gitignorePath)) {
    await fs.writeFile(gitignorePath, `${GITIGNORE_ENTRIES.join('\n')}\n`);
    return;
  }

  const content = await fs.readFile(gitignorePath, 'utf8');
  await fs.writeFile(gitignorePath, appendTaskFlowEntries(content));
}

module.exports = {
  GITIGNORE_ENTRIES,
  stripTaskFlowGitignoreLines,
  appendTaskFlowEntries,
  updateGitignore,
};
